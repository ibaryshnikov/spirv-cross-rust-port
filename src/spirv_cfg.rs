...
impl<'a> Cfg {
    ...
    fn get_immediate_dominator(&self, block: u32) -> u32 {
        let itr = self.immediate_dominators
            .values()
            .find(|x| x == block);
        if let Some(value) = itr {
            return *value;
        }
        0
    }
    ...
    fn build_immediate_dominators(&mut self) {
        ...
        self.immediate_dominators.insert(func.entry_block, func.entry_block);

        for block in self.post_order.iter().rev() {
            if let Some(pred) = &self.preceding_edges.get(block) {
                for edge in pred {
                    if self.immediate_dominators.get(block) {
    ...
    fn is_back_edge(&self, to: u32) -> bool {
        ...
        let itr = self.visit_order.values().find(|x| x == to);
        if let Some(value) = itr {
            return value.v == 0;
        }
...
//        return itr->second.get() == 0;
    ...
    fn get_preceding_edges(&self, block: u32) -> &'a Vec<u32> {
        let itr = self.preceding_edges
            .values()
            .find(|x| x == block);
        if let Some(edge) = itr {
            return edge;
        }
        &vec![]
    }

    fn get_succeeding_edges(&self, block: u32) -> &'a Vec<u32> {
        let itr = self.succeeding_edges
            .values()
            .find(|x| x == block);
        if let Some(edge) = itr {
            return edge;
    ...
    fn walk_from<T>(&self, seen_blocks: &mut HashSet<u32>, block: u32, op: &T) {
...
impl<'a> DominatorBuilder {
    ...
    fn lift_continue_block_dominator(&self) {
        ...
        let block = &self.cfg.get_compiler().get::<SPIRBlock>(dominator);
        let post_order = self.cfg.get_visit_order(dominator);
        ...
        if back_edge_dominator {
            dominator = self.cfg.get_function().entry_block;
        }
    }
}
